...
var moduleFunction = function(args) {
	...
	const studentIdBlock = function() {
		const block = [];
		block.push({text: student('first') + ' ' + student('middle') + ' ' + student('last'),bold: true,fontSize: 14});
		block.push({text: student('gender', [wrapWith(' (', ')'), mandatoryNewLine])});

		block.push({text: student('phone', addNewLine),fontSize: 12});
	...
	const guardianOneBlock = function() {
		const block = [];
		block.push({text: student('gOneRelationship', wrapWith('', ': ')),bold: true,fontSize: 14});
		block.push({text: student('gOneName', addNewLine),bold: true,fontSize: 14});
		block.push({text: student('gOnePhoneMain', addSpace),bold: false,fontSize: 12});
	...
	const guardianTwoBlock = function() {
		const block = [];
		block.push({text: student('gTwoRelationship', wrapWith('', ': ')),bold: true,fontSize: 14});
		block.push({text: student('gTwoName', addNewLine),bold: true,fontSize: 14});
		block.push({text: student('gTwoPhoneMain', addSpace),bold: false,fontSize: 12});
	...
	const finalDocSpec = {
		...
		body: [
			...
			[assembleTextCell('text', 'ICD-9-CM is some long blab I don\'t know about', '', '3', '', 'left'),
